...
1. Input and Output layers communicate with the AI system through defined data contracts.
2. Agent does not depend directly on a concrete Model, Memory, or Tool implementation.
3. AI Core, Memory, and Tools are replaceable components behind interfaces.
4. Personality receives the result/context required for expression; it does not become the reasoning engine.
5. Avatar belongs to the output/presentation side and must not be responsible for conversation reasoning or personality data storage.
6. Configuration and logging are shared infrastructure and must not contain AI decision logic.
7. Concrete implementations may depend on interfaces, but interfaces must not depend on concrete implementations.
8. A lower-level component must not call a higher-level component merely to bypass the defined flow.
...
- add Voice or Avatar output without changing the reasoning pipeline.

## 7. Scope of this architecture
...
Those components are implemented in their respective later parts of Part III.
