...
class AgentPermissionChecker:
    ...
    def check(
        ...
        return self.repository.is_allowed(agent_id=agent_id, tool_id=tool_id)
    ...
    def require(
        ...
        if not self.check(agent_id=agent_id, tool_id=tool_id):
            raise ToolPermissionDenied("Agent is not allowed to use this Tool")